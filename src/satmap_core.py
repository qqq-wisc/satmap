...
from common import compose_swaps, extract2qubit, extract_qubits
                ...
                for j in liveLog:
                    for prev in range(layers[k-1], layers[k]):
                        ...
                        for current in currentRange:
                            writeHardClause(path, top, swapLits + [(False, "x", i, j, prev), (True, "x", perm[i], j, current)], physNum, logNum, numCnots, swapNum, satSolver=satSolver)
                            writeHardClause(path, top, swapLits + [(True, "x", i, j, prev), (False, "x",perm[i], j, current)], physNum, logNum, numCnots, swapNum, satSolver=satSolver)
...
def solve(progName, cm, swapNum, chunks, iterations=100, time_wbo_max = 3600, initial_map=None, qaoa=False, _routing=True, _weighted=False, _calibrationData=None,  pname="test", sname="out"):
    ...
    while currentChunk < chunks:
        ...
        if currentChunk == 0:
            ...
            try:
               p = subprocess.Popen(["/home/lib/Open-WBO-Inc/open-wbo-inc_release",   "-iterations="+str(iterations), pname+"-chnk"+str(currentChunk)+".cnf"],  stdout=open( sname + "-chnk0" + ".txt", "w"))
            ...
            if time_wbo_max:
                solve_time_rem = time_wbo_max- time_elapsed_wbo 
            try:
                p = subprocess.Popen(["/home/lib/Open-WBO-Inc/open-wbo-inc_release", "-iterations="+str(iterations), pname+"-chnk"+str(currentChunk)+".cnf"], stdout=open(sname + "-chnk" + str(currentChunk) + ".txt", "w"))
                p.wait(timeout=solve_time_rem/(chunks-currentChunk))
            except subprocess.TimeoutExpired:
                print("exiting open-wbo because of solve time alloted...")
                p.terminate()
